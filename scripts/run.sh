...
[ -x "scripts/common/e2e_test_plan_review_loop.sh" ] || die "scripts/common/e2e_test_plan_review_loop.sh 가 없거나 실행 권한이 없습니다"
...
rm -f PLAN.md REVIEW.md IMPL.md CODE_REVIEW.md \
      E2E_TEST_PLAN.md E2E_TEST_PLAN_REVIEW.md E2E_TEST_IMPL.md E2E_TEST_RESULT.md E2E_TMPDIR.txt
...
if [[ $PHASE3_EXIT -ne 0 ]]; then
    ...
    FRESH=1 scripts/common/e2e_test_plan_review_loop.sh "$ISSUE_ID" || PHASE4_EXIT=$?
...
if [[ "$ISSUE_TYPE_FOR_E2E" == "feature" || "$ISSUE_TYPE_FOR_E2E" == "refactor" ]]; then
    ...
    echo "  - E2E_TEST_PLAN_REVIEW.md: $WORKTREE_DIR/E2E_TEST_PLAN_REVIEW.md"
