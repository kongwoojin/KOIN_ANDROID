...
# ──────────────────────────────────────────────
# GitHub 이슈(feature) 분석 → PLAN.md 생성
# ──────────────────────────────────────────────

echo "📋 GitHub 이슈(feature) 분석 중..."

SYSTEM_PROMPT_FILE="$PROJECT_ROOT/scripts/prompts/feature-plan.md"
...

{
    ...
    printf -- '- 신규 브랜치 명: `feature/%s` 또는 `feature/%s-{N}`\n' "$ISSUE_ID" "$ISSUE_ID"
    printf -- '- 이슈 타입은 feature 로 확정되어 있으며 재분류하지 않습니다.\n'
