...
- Read the root `AGENTS.md` and, if any module is involved, that module's `<module>/AGENTS.md`, with the Read tool. Translate the UI-technology split, the Compose two-function pattern, the `Result<T>`/`Pair` split, and the 10 Critical Rules into concrete implementation directives in PLAN.md. Preserving the existing pattern is especially important for verifying "no behavior change".
...
2. The issue type is fixed as refactor — do not reclassify.
3. Branch name MUST be `refactor/{issue-number}` or `refactor/{issue-number}-{N}`.
...
12. Behavior MUST NOT change before vs. after the refactor.
13. Files outside the refactor target MUST NOT be modified.
14. Search for analogous existing implementations in the codebase via Grep/Read first. Investigate how other modules use the same core class / utility / pattern, record the validated usage pattern in PLAN.md, and direct the implementer to follow it. Always check existing patterns before inventing a workaround. If no reference implementation exists for a brand-new domain, state "참고 구현 없음 (신규 도메인)" in PLAN.md and design from scratch.
...
15. Java files (`.java`) are legacy — exclude them from analysis, edits, and references. Review only Kotlin (`.kt`).
...
# Rules
1. 리팩토링 전후로 동작에 차이가 있어서는 안됩니다.
2. 리팩토링 대상이 아닌 파일을 수정하는 것은 안됩니다.
...
- 리팩토링 범위에 대해 작성합니다.
### 리팩토링 대상
- koin/src/main/java/in/koreatech/koin/MainActivity.kt
### 리팩토링 목표
- View-based Screen을 Compose로 리팩토
</example>
