...
- Read the root `AGENTS.md` and, if any module is involved, that module's `<module>/AGENTS.md`, with the Read tool. Translate the UI-technology split, `Result<T>`/`Pair` split, the 10 Critical Rules, and the commit convention into concrete implementation directives in PLAN.md.
...
2. The issue type is fixed as bugfix — do not reclassify.
3. Branch name MUST be `fix/{issue-number}`.
