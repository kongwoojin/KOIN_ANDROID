...
# ──────────────────────────────────────────────
# GitHub 이슈(bugfix) 분석 → PLAN.md 생성
# ──────────────────────────────────────────────

echo "📋 GitHub 이슈(bugfix) 분석 중..."

SYSTEM_PROMPT_FILE="$PROJECT_ROOT/scripts/prompts/bug-plan.md"
...

{
    ...
    printf -- '- 신규 브랜치 명: `fix/%s`\n' "$ISSUE_ID"
    printf -- '- 이슈 타입은 bugfix 로 확정되어 있으며 재분류하지 않습니다.\n'
